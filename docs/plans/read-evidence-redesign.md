...
| 2 | 真实 writer 证据要求与统一预检 | 已完成 Go 侧（85334ef34）；shell/MCP 未知写范围与多目标批次预检未覆盖 |
| 3 | 预算、有界续读及旧回执兼容 | 预算与阶梯已完成（72ddddfe2）；旧回执适配器未接 |
| 4 | 结构化事件、单卡 UI、跨端与本地化 | Go 侧事件已完成（72ddddfe2）；前端状态宿主、本地化与无障碍未开始 |
| 5 | 默认启用新协调器，移除冲突的旧执行路径 | 未开始 |
| 6 | 方案文档、工具说明、兼容说明与验收记录 | 方案文档已更新；工具说明与验收记录待补 |

默认行为尚未切换：新协调器仍只以 shadow 方式运行，`inspect` 仍被旧状态机当作隐式全文读取。步骤 5 完成前不满足"用户无需配置即获得新行为"。
