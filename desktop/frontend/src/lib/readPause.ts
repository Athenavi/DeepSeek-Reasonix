...
export function readPauseItem(pause: WireReadPause | undefined, fallbackID: string): Item {
  ...
  return { kind: "notice", id: pause?.id ? `read-pause-${pause.id}` : fallbackID, level: "info", code: "incomplete_read", title: t("notice.readPauseTitle"), text: t("notice.readPauseBody"), detail: details.join("\n") };
}
