...

{
  ...
  assert.equal(fake.calls.at(-1), "activate null", "hiding the panel deactivates the native view");
  store().setVisible(true);
  assert.equal(fake.calls.at(-1), "activate a1");
  ...
  assert.equal(fake.calls.at(-1), "activate null", "detach releases the native view");
...

{
  ...
  assert.equal(fake.calls.at(-1), 'navigate seed {"url":"http://localhost:3000/app"}', "Enter navigates the active tab");
  ...
  assert.equal(fake.calls.at(-2), `open https://Example.org ${USER_TASK_ID} false`, "+ opens the draft as a user tab");
  assert.equal(store().activeTabId, "new:https://Example.org");
  assert.equal(fake.calls.at(-1), "activate new:https://Example.org");
  store().clearDraft(store().activeTabId);
  fake.emitTabs([]);
  store().setDraft(null, "wiki.example");
  await store().submitAddress();
  assert.equal(fake.calls.at(-2), `open https://wiki.example ${USER_TASK_ID} false`, "Enter without a tab opens one");
  await store().zoom(store().activeTabId!, 1);
  assert.equal(fake.calls.at(-1), `zoom ${store().activeTabId} 1.1`);
