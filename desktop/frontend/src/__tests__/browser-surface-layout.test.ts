...
flush();
assert.deepEqual(layouts.at(-1), { x: 10, y: 60, width: 300, height: 200 }, "scrolling an ancestor re-measures");
box = { ...box, width: 420 };
view.dispatchEvent(new view.Event("resize"));
flush();
assert.deepEqual(layouts.at(-1), { x: 10, y: 60, width: 420, height: 200 }, "window resize re-measures");
box = { ...box, left: 5 };
observer.trigger();
detachLayout();
assert.equal(cancelled, 1, "a pending frame is cancelled on detach");
assert.equal(layouts.at(-1), null, "detach hides the native view");
...
await settle();
flush();
assert.deepEqual(overlays.at(-1), true, "toggling the attribute on an existing element counts");
detachOverlay();
assert.deepEqual(overlays.at(-1), false, "detach clears an active overlay state");
plain.remove();
await settle();
flush();
